...
void RectangleClickEvent::onClicked(int qmlClickedDataNum)
{
    //fistlabel objectname을 찾음
    QObject* firstlabeltext = m_engine->rootObjects().first()->findChild<QObject*>("first_label_text");
    QObject* seconedlabeltext = m_engine->rootObjects().first()->findChild<QObject*>("seconed_label_text");
    ...
    m_num1.append(QString::number(qmlClickedDataNum));

    //string to int 형변환 방법
    int stringchangeint = m_num1.toInt();

    //firstlabeltext  text property에 data를 입력
    firstlabeltext->setProperty("text",stringchangeint);
    seconedlabeltext->setProperty("text",stringchangeint);
    switch (qmlClickedDataNum) {
    case 10:
        //연산자
    case 11:
        //결과값 =클릭
